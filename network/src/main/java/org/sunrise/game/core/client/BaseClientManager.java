...
public class BaseClientManager {
    ...
    public static void sendToServer(BaseMessage message) {
        ...
        if (baseClient != null) {
            ...
            LogCore.BaseClient.warn("BaseClient sendToServer fail, use NodeId = { {} }, message = {{}}", message.getNodeId(), message);
        }
    }
    ...
    /**
     * 停机时关闭所有已注册的 BaseClient。
     */
    @OnShutdown(order = 80)
    public static void shutdownAll() {
        for (Map.Entry<String, BaseClient> entry : baseClients.entrySet()) {
            try {
                entry.getValue().onStop();
            } catch (Exception e) {
                LogCore.BaseClient.error("BaseClientManager shutdownAll error, nodeId={}: {}",
    ...
    public static void recvFromServer(String nodeId, Object data) {
        ...
        if (baseClient != null) {
            ...
            LogCore.BaseClient.warn("BaseClient recvFromServer fail, use NodeId = { {} }, message = {{}}", nodeId, data);
        }
    }
}
